...
public class Cannone extends Componente implements Ruotabile {
	...
	public boolean posizionabile(Nave nave, int x, int y) {
		...
		if (punta == Orientazione.NORD && nave.getComponente(y, x - 1) != null) {
			return false;
		} else if (punta == Orientazione.EST && nave.getComponente(y + 1, x) != null) {
			return false;
		} else if (punta == Orientazione.SUD && nave.getComponente(y, x + 1) != null) {
			return false;
		} else {
			if (nave.getComponente(y - 1 , x) != null)
				return false;
